...
const itemPlayer = document.querySelector('.item_player2');
let texto = "";


const players = document.querySelectorAll('.item_player1');
...
players.forEach(player => {
    player.addEventListener("click", () => {
        let attr = player.getAttribute('data-name');
        validar(attr)
...
function validar(player) {
    ...
    for (let i = 0; i < items.length; i++) {
        if (random === i) {
            let item = items[i];
            inner += `
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="${item.vb}" class="icon">
                        <path
                            d="${item.path}" />
                    </svg>
                    `;

            itemPlayer.innerHTML = inner;
            if (player === item.name) {
                texto = "Empate con " + player;
            } else if
                (
                (player === "tijeras" && item.name === "papel") ||
                (player === "piedra" && item.name === "tijeras") ||
                (player === "papel" && item.name === "piedra")
            ) {
                texto = "Ganaste con " + player;
            } else {
                texto = "Perdiste contra " + item.name;
            }
...
function cargar() {
    itemPlayer.innerHTML = `
                    <svg xmlns="http://www.w3.org/2000/svg" class="question" viewBox="0 0 320 512">
                    <path
